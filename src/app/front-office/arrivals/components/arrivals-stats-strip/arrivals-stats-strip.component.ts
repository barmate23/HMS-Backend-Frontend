...
import { ArrivalStatus } from '../../models/arrival.model';

@Component({
  selector: 'app-arrivals-stats-strip',
  standalone: true,
        ...
        >
          ...
          <div class="flex flex-col">
            ...
            <div class="flex items-baseline gap-1">
              ...
              <span class="text-[10px] font-bold text-slate-400">Guests</span>
            </div>
          </div>
...
export class ArrivalsStatsStripComponent {
  @Input() counts: { total: number; checkedIn: number; pending: number; noShow: number } = { total: 0, checkedIn: 0, pending: 0, noShow: 0 };
  @Output() statClicked = new EventEmitter<ArrivalStatus | 'all'>();

  get stats() {
    return [
      { label: 'Total Arrivals', icon: 'person_add', count: this.counts.total, status: 'all' as const, colorClass: 'bg-[#1A3C5E]' },
      { label: 'Checked In', icon: 'login', count: this.counts.checkedIn, status: 'checked-in' as const, colorClass: 'bg-[#2D9E6B]' },
      { label: 'Pending', icon: 'schedule', count: this.counts.pending, status: 'pending' as const, colorClass: 'bg-[#F4A261]' },
      { label: 'No Show', icon: 'person_off', count: this.counts.noShow, status: 'no-show' as const, colorClass: 'bg-[#E63946]' }
    ];
  }
}
